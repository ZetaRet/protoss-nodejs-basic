...
function ListDir(serverobj, path, dir, config) {
	return serverobj.addParamsPathListener(
		...
		(server, robj, routeData, request, response) => {
			...
			var c, ccn, stats, cadata, filename;
			var mainid = fileid;
			var unpack = fileid.split(".");
			var ext = unpack.pop() || "";
			var prefix = unpack[0] || "";
			var hash = config.hashMap && config.hashMap[prefix] ? unpack.pop() || "" : null;
			...
			if (!nofile && fs.existsSync(filename)) {
				...
				function endResponse() {
					...
					if (ccn && (!config.nocache || !config.nocache[mainid]))
						response.setHeader("Cache-Control", "max-age=" + ccn);
